...
import {Alert, Box, Button, Card, CardContent, Container, Dialog, DialogActions, DialogContent, DialogTitle, Divider, FormControl, Grid, IconButton, InputLabel, MenuItem, Paper, Select, Stack, TextField, Typography,} from '@mui/material';
import {Add, ArrowBack, Delete, Edit, Person, Save,} from '@mui/icons-material';
...
const CreateGroup = () => {
    ...
    const handleMemberChange = (e) => {
        ...
        if (name.startsWith('address.')) {
            ...
            setCurrentMember({
                ...currentMember,
                unit: selectedUnit ? {id: selectedUnit.id} : null,
            });
        } else {
            setCurrentMember({
                ...currentMember,
                                            ...
                                            <Card className="member-card">
                                                <CardContent>
                                                    ...
                                                    <Stack spacing={0.5} className="member-info">
                                                        ...
                                                        {member.unit && member.unit.id && (
                                                            <Typography variant="body2" color="textSecondary">
                                                                <strong>Einheit:</strong> {
                                                                (() => {
                                                                    const unit = units.find(u => u.id === member.unit.id);
                                                                    return unit ? `${unit.name}${unit.city ? ` - ${unit.city}` : ''}` : `Unit ${member.unit.id}`;
                                                                })()
                                                            }
                                                            </Typography>
                                                        )}
                    ...
                    <DialogContent dividers>
                        <Box sx={{py: 1}}>
                            ...
                            <Grid container spacing={2}>
                                <Grid item xs={12}>
                                    <FormControl fullWidth variant="outlined">
                                        <InputLabel id="member-unit-select-label">Einheit (optional)</InputLabel>
                                        <Select
                                            labelId="member-unit-select-label"
                                            id="member-unit-select"
                                            name="unitId"
                                            value={currentMember.unit?.id || ''}
                                            onChange={handleMemberChange}
                                            label="Einheit (optional)"
                                            style={{width: '10rem'}}
                                            MenuProps={{
